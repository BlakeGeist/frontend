...
const BELBOON_MATCH = /^(http:\/\/.+belboon.de\/adtracking\/\w+\.html)(\/subid=([^\/]+)\/?)?/;
...
function fixTradeDoubler(s_url) {
  if (/p=\d+/.test(s_url)) {
    return byParameter('epi')(s_url);
  }
  else if (/p\(\d+\)/.test(s_url)) {
    return s_url.replace(/epi\([^\)]+\)/, '') + 'epi({{SubId}})';
  }
  return '';
}
...
function fixTradeTracker(s_url) {
  ...
  } // 2. directlinking technique (pointing to the advertiser’s site directly) has the reference behind the 3rd underscore.
