...
function * getMerchantBySlug (merchantSlug, lang, region, ctx) {
  let result = yield dataClient.get('/getMerchantBySlug/' + merchantSlug + '/', {lang: lang, region: region}, ctx);
...
function * getMerchantsByIds (ids, lang, region, active, ctx) {
  ids = _.uniq(_.remove(ids, _.isFinite));
  if (_.isEmpty(ids)) {
    return ids;
  }
  let result = yield dataClient.get('/getMerchantsByIds/', {ids: ids, lang: lang, region: region, active: active}, ctx);
  return result.body;
}
...
function * getRankedListOfMerchants (selectors, ctx) {
  ...
  'userRegion', 'isNationalStore', 'isHighestCashbackOrder', 'hasFlat', 'hasPercentage' ,'isFavStore', 'nameFragment']);
