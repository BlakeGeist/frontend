...
function * getPromoCode (code, bypassCache, ctx) {
  console.log(" iam called in tils",code);
  let result = yield dataClient.get(helpers.setRequestPayload('/getPromoCode/' + code + '/' + toQueryString({bypass: bypassCache}), ctx));
  ctx.assert(result.statusCode !== 404, 404, 'User not found', {internalCode: 110});
