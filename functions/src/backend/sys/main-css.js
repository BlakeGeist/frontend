...
function * renderCss (cssFile) {
  ...
  if (!MAIN_CSS_CACHE[WL][cssFile]) {
    ...
    try {
      const overStat = yield fs.stat(lessMainOverride);
    ...
    lessString += yield fs.readFile(lessFile, 'utf8');

    try {
      const out = yield less.render(lessString, {
        filename: lessMain,
        paths: [lessRoot],
...
function * mainCss () {
  yield renderCss.call(this, 'main.less');
}

function * coreCss () {
  yield renderCss.call(this, 'core.less');
}

function * fillCaches () {
  yield _.flatten(Object.keys(configs.whitelabel)
    .filter(k => k !== 'defaults')
    .map(whitelabel => {
      const ctx = {
        state: {
